...
const ServiceDetailCard = ({ cardData }: Pros) => <section
    ...
    {cardData.description && <p className=' font-semibold my-6'>{cardData.description}</p>}
    <ul className='flex flex-wrap justify-center items-center gap-8 my-6'>
        {cardData.data.length > 0 && cardData.data.map((v, i) => <li key={i} className='w-96 h-80 border border-gray-600 p-4 md:p-8 rounded-lg bg-gray-200'>
